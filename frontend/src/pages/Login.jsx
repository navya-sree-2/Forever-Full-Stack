    ...
    return (
        <form onSubmit={onSubmitHandler} className='flex flex-col items-center w-[90%] sm:max-w-96 m-auto mt-14 gap-4 text-gray-800'>
            ...
            <div className='w-full flex justify-between text-sm mt-[-8px]'>
                ...
                {
                    currentState === 'Login' ?
                        <p className='cursor-pointer'>Don't have an account? <span onClick={() => setCurrentState('Sign Up')} className='font-semibold text-gray-700'>Sign Up</span></p>
                        : <p className='cursor-pointer'>Already have an account? <span onClick={() => setCurrentState('Login')} className='font-semibold text-gray-700'>Login</span></p>
                }
            </div>
